 ...
 */
async getTaskById(boardId:string, taskId:string):Promise<TaskEntity | undefined>{
  return await this.taskRepository.findOne({ boardId, id:taskId}) as TaskEntity | undefined;
}
...
async deleteTaskById(boardId:string, taskId:string):Promise<boolean>{
  return !!(await this.taskRepository.delete({ id:taskId, boardId})).affected
} 
