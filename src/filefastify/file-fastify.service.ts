import {HttpException, BadRequestException, Injectable, StreamableFile} from '@nestjs/common';
...
export class FilefastifyService {
  ...
  async handler(_field: string, file: string, filename: string, _encoding: string, _mimetype: string): Promise<void> {
    const pipeline = util.promisify(stream.pipeline);
    const pathFile = path.join(__dirname, `../../public/${filename}`) ;
    const writeStream = createWriteStream(pathFile);
    await pipeline(file, writeStream).catch(err => {throw new HttpException('Internal server error - pipeline fiald', 500)});
  }
